...
export default async function post(request, response) {
  ...
  try {
    ...
    await client.query(
      `INSERT INTO Team (Title, Abstract, Link, Autor, State, Date, Text, Image) VALUES ($1, $2, $3, $4, $5, $6, $7, $8);`,
      [title, abstract, link, autor, state, date, text, imageBytes]
    );

    return response.status(201).json({ message: 'Team added successfully' });
  } catch (err) {
